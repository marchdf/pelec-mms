...
if __name__ == '__main__':
    ...
    for k, res in enumerate(resolutions):
        fname = os.path.join(os.path.abspath(str(res)), 'datlog')
        df = pd.read_csv(fname, delim_whitespace=True)

        rho0, u0, p0, L = get_ic(os.path.abspath(str(res)))

        idx = 100
        print('Loading {0:d} at t = {1:e}'.format(res, df['time'].iloc[idx]))
        lst.append([res,
                    1. / res,
    ...
    # Theoretical error
    order = 2.0
    edf['rho_theory'] = edf['rho_mms_err'].iloc[0] * \
        (edf['resolution'].iloc[0] / edf['resolution'])**order
    edf['u_theory'] = edf['u_mms_err'].iloc[0] * \
        (edf['resolution'].iloc[0] / edf['resolution'])**order
    edf['v_theory'] = edf['v_mms_err'].iloc[0] * \
        (edf['resolution'].iloc[0] / edf['resolution'])**order
    edf['w_theory'] = edf['w_mms_err'].iloc[0] * \
        (edf['resolution'].iloc[0] / edf['resolution'])**order
    edf['p_theory'] = edf['p_mms_err'].iloc[0] * \
        (edf['resolution'].iloc[0] / edf['resolution'])**order
